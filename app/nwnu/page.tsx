...
const FAQ_ITEMS = [
  { q: "为什么只支持 .edu.cn 邮箱？", a: "为了确保圈子真实、安全。我们仅限兰州本地高校学生，避免校外人员和虚假身份混入。" },
...
export default function NWNUPage() {
  ...
  return (
    <PageShell backgroundImage backgroundSrc="/nwnu_background.jpg" imageOpacity={0.15} className="flex flex-col items-center justify-start text-[#f0ebe4]">
            ...
            <motion.div key="hero" exit={{ opacity: 0, y: -16, transition: { duration: 0.25 } }} className="w-full flex flex-col items-center">
              <section className="min-h-screen flex flex-col items-center justify-center gap-8 md:gap-12 p-6 pt-16 pb-12">
                <div className="space-y-8 max-w-2xl">
                  <motion.div variants={fadeUp} initial="hidden" animate="show" className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white/[0.10] border border-white/[0.15] backdrop-blur-lg shadow-[0_2px_8px_rgba(0,0,0,0.2)]">
                    ...
                    <span className="text-[10px] uppercase tracking-[0.3em] text-zinc-300 font-semibold">西北师范大学 · 专属匹配</span>
                  </motion.div>

                  <motion.h1 variants={fadeUp} initial="hidden" animate="show" className="relative space-y-2">
                    <div className="text-lg text-zinc-400 font-light tracking-wider">西北师范大学</div>
                    <div className="text-6xl sm:text-8xl md:text-9xl lg:text-[10rem] font-black tracking-tighter select-none leading-[0.85]">
                      ...
                      <span className="bg-gradient-to-r from-rose-400 via-pink-400 to-rose-300 bg-clip-text text-transparent">DATE</span>
                    </div>
                  </motion.h1>
                  ...
                  <motion.div variants={fadeUp} initial="hidden" animate="show" className="inline-flex items-center gap-3 px-5 py-2 rounded-full bg-white/[0.10] border border-white/[0.15] backdrop-blur-xl shadow-[0_4px_16px_rgba(0,0,0,0.3)]">
                    <svg className="w-3.5 h-3.5 text-rose-400/70" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 6v6h4.5m4.5 0a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                    ...
                    <span className="text-xs font-mono text-rose-300 tabular-nums">{countdown.days}<span className="text-zinc-500 mx-0.5">d</span>{countdown.hours}<span className="text-zinc-500 mx-0.5">h</span>{countdown.mins}<span className="text-zinc-500 ml-0.5">m</span></span>
                  </motion.div>
                  ...
                  <motion.div variants={fadeUp} initial="hidden" animate="show" className="w-full max-w-md mx-auto space-y-3 pt-4">
                    <div className="relative">
                      ...
                      <input id="email" type="email" value={email}
                        ...
                        placeholder="输入你的 nwnu.edu.cn 校园邮箱"
                        className={`w-full pl-12 pr-5 py-4 rounded-2xl bg-white/[0.12] backdrop-blur-2xl border text-base transition-all duration-300 focus:outline-none placeholder:text-zinc-400 ${emailError ? "border-red-400/40" : "border-white/[0.18] focus:border-rose-400/60 hover:border-white/[0.25]"}`}
                      />
                    </div>
                    ...
                    <button onClick={handleStart} className="group relative w-full px-6 py-4 rounded-2xl bg-gradient-to-r from-rose-400 to-pink-400 text-white font-bold text-lg active:scale-[0.98] transition-all duration-300 overflow-hidden shadow-[0_4px_32px_rgba(251,113,133,0.4)] hover:shadow-[0_8px_48px_rgba(251,113,133,0.55)]">
              ...
              <section className="w-full py-14 md:py-20 px-4 sm:px-6 border-t border-white/[0.06]">
                <div className="max-w-3xl mx-auto text-center space-y-4">
                  <h2 className="text-3xl font-bold tracking-tight">欢迎 <span className="text-rose-400 font-serif-cn">NWNUer</span></h2>
                  <p className="text-zinc-400 text-sm font-light">仅限 @nwnu.edu.cn 邮箱认证 · 师大专属匹配池</p>
                </div>
              </section>
              ...
              <footer className="w-full py-12 px-6 border-t border-white/[0.06]">
                <div className="max-w-5xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4 text-xs text-zinc-400 tracking-wider">
                  <div className="flex items-center gap-6"><span>Hover DATE · 西北师范大学</span><span className="text-zinc-600">|</span><span>© 2026 Lanzhou College Alliance</span></div>
            ...
            <motion.div key="welcome" initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, y: -12 }} className="min-h-screen flex flex-col items-center justify-center p-6">
              <div className="bg-white/[0.06] backdrop-blur-3xl border border-white/[0.15] rounded-[2rem] p-10 sm:p-14 max-w-lg w-full text-center space-y-8 shadow-[0_8px_60px_rgba(0,0,0,0.5)]">
                ...
                <div className="space-y-2 text-left max-w-xs mx-auto">
                  {["5 个维度 · 22+ 道题目", "大约需要 5-8 分钟", "答案自动保存，随时继续"].map((t, i) => (<div key={i} className="flex items-center gap-3 text-sm text-zinc-400"><svg className="w-4 h-4 text-rose-400/60 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>{t}</div>))}
                </div>
